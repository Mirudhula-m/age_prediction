"""
DENOISING CRITERION WITH STANDARD VARIATIONAL AUTOENCODER
...
class denoise_Autoencoder(nn.Module):
    ...
    def encode(self, x):
        normal = torch.distributions.Normal(0, 5)
        x_noise = x + normal.sample(sample_shape=torch.Size(x.shape)).to(device)
        self.encode_out = self.encodingLayers(x_noise)
...
model = denoise_Autoencoder(inpd, hiddenLayers_Autoencoder, latentDim).to(device)
...
for epoch in range(N_epochs):
    ...
    for batch in range(int(N_batches)):
        ...
        # reconstruction loss
        batch_loss = DVAElossFunc(recon_batch, x, mu, logvar, KL_weight)
        # batch_loss.backward()
...
_,y = data.GetTestData() # test data.
# ages.
...
temp = pd.DataFrame(data = np.hstack((pred,gndt,data.reshape(inpd,1))), 
    columns=["predicted_age", "Age", "ID"])
temp.to_csv("VAEpredictions_mod1_test.csv")
