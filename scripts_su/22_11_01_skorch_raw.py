...
class MyModule(nn.Module):
    def __init__(self, n_in = n_genes, n_h=n_h, n_layers=2, dropout=0.1):
        super().__init__()
        self.n_in = n_in
...
test_df = pd.read_csv(data_dir+ "22_11_02_df_val.csv")
...
##### import folds.
#splits = pickle.load(open(data_dir+ '22_09_20_SPLIT_IDS.pkl', 'rb'))
...
cts= scaler.fit_transform(train.X) # it should be fit to the train data, and used on the test.
cts_test =scaler.transform(test.X).astype(np.float64)
...
# prediction task -------------------------------------------------------
#===============================================================================
datestring = "22_11_22" # for naming outputs.
...
X=torch.tensor(cts.todense()).to(torch.float32)
X_test = torch.tensor(cts_test.todense(), dtype=torch.float32)
...
net.fit(X,y)
# PICKLE FILE
with open(out_dir +datestring+'skorch_net_weighted_'+str(n_h)+'.pkl', 'wb') as f:
    pickle.dump(net, f)

net.save_params(f_params=out_dir +datestring+'_skorch_net_weighted_'+str(n_h)+'_params.pkl')
...
temp.to_csv(out_dir +datestring+'_skorch_net_weighted_'+str(n_h)+"train_pred.csv")
...
temp.to_csv(out_dir +datestring+'_skorch_net_weighted_'+str(n_h)+"test_pred.csv")
